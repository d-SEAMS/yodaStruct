...
- This repository, with work from the repository root so the dump
  path resolves
- The :doc:`../quickstart` build, or the meson / Nix steps in Step 1
...
If you already ran the :doc:`../quickstart`, the exports (or the Nix
shell) are enough. Skip the compile.
...
call are on ``dseams.core``; see :doc:`../reference/lua`.
...
``chill-plus``, is in :doc:`../howto/fennel`.
...
``require("dseams")``. See :doc:`../explanation/library-not-cli`.

Next Steps
==========

- :doc:`../howto/fennel` :: Lisp syntax on the same table
- :doc:`../howto/embed-lua` :: Load the ``.so`` from your own process
- :doc:`../reference/lua` :: Helpers and ``dseams.core`` names
