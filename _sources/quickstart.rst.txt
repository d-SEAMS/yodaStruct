...
in `Install the library <howto/install.rst>`_.
...
``lua/dseams.fnl`` is in `Fennel <howto/fennel.rst>`_.
...
`Install the library <howto/install.rst>`_
    How the ``.so`` is built and found

`Embed in a host <howto/embed-lua.rst>`_
